    ...
    consultation: [
        {
            ...
            next: [{ code: "registered", title: "Зарегистрирован", requires_reason: false }]
        },
        {
            code: "registered",
            title: "Зарегистрирован",
            next: [
                { code: "sent_to_other_park", title: "Отправлен в другой парк", requires_reason: true },
                { code: "thinking", title: "Клиент думает", requires_reason: false },
                { code: "no_answer", title: "Клиент не отвечает", requires_reason: false },
                { code: "incorrect_data", title: "Неверные данные", requires_reason: true },
        ...
        },
        {
            ...
            next: [
                { code: "registered", title: "Зарегистрирован", requires_reason: false },
                { code: "rejected", title: "Отклонен", requires_reason: true }
            ]
        },
        {
    ...
    taxiPark: [
        {
            code: "pending",
            title: "Ожидание обработки",
            next: [{ code: "sent_to_partner", title: "Отправлен партнеру", requires_reason: false }]
        },
        {
            code: "sent_to_partner",
            title: "Отправлен партнеру",
            next: [
                { code: "partner_error", title: "Ошибка при отправке партнеру", requires_reason: true },
                { code: "approved", title: "Подключен", requires_reason: false },
                { code: "rejected", title: "Отклонен", requires_reason: true }
            ]
        },
...
const EditFormModal = ({ open, onClose, record, refreshData }) => {
    ...
    const fetchStatusHistory = async () => {
        try {
            const response = await axios.get(`${API_URL}/forms/${record.id}/statusHistory`);
    ...
    useEffect(() => {
        if (record?.statusCode) {
            const currentStatus = statuses.taxiPark.find((s) => s.code === record.statusCode);
            if (currentStatus) {
    ...
    return (
        <Modal open={open} onCancel={handleClose} title="Просмотр и редактирование" footer={null}>
            <Form form={form} layout="vertical" initialValues={record} onFinish={handleUpdate}>
                ...
                {requiresReason && (
                    <Form.Item name="reason" label="Причина смены статуса" rules={[{ required: true, message: "Введите причину!" }]}>
                        <Input.TextArea rows={3} placeholder="Введите причину смены статуса" />
                    </Form.Item>
                )}
                        ...
                        {history.map((item) => {
                            ...
                            return (
                                <Timeline.Item key={item.id}>
                                    ...
                                    {item.reason && <div style={{ color: "#888" }}>Причина: {item.reason}</div>}
                                </Timeline.Item>
                            );
                        })}
                    </Timeline>
